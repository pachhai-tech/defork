...
export function AIImage({ onResult }: { onResult: (blob: Blob, modelId: string) => void }) {
  const toast = useToast()
  const [loading, setLoading] = useState(false)
  const [progress, setProgress] = useState<number>(0)
  const [prompt, setPrompt] = useState('IMF dossier style portrait, cinematic lighting')

  async function run() {
    setLoading(true)
    try {
      toast?.show?.({ message: 'Loading image model…', type: 'info' })
      const mod = await import(/* @vite-ignore */ 'https://unpkg.com/@mlc-ai/web-stable-diffusion@0.1.1/dist/index.esm.js')
